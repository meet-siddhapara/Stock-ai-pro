...
if(feature =="Home"):
    ...
    st.subheader("Your All-in-One Financial Assistant for Retail Investors")
    st.markdown("Gain insights, analyze technicals, screen stocks, recognize patterns, and forecast next-day prices for all National Stock Exchange (NSE) listed stocks.")
    ...
    col1.metric("Listed Companies", "1,773", "NSE")
    col2.metric("Historical Data", "10 Years", "Daily")
    col3.metric("Technical Indicators", "11+", "Tools")
    col4.metric("ML Forecasting", "Next Day", "Prediction")

    st.divider()

    # Interactive Feature Explorer
    st.header("✨ Explore Features")
    st.markdown("Click on any feature below to learn more about what you can do:")
    ...
    with tab1:
        st.markdown('''
        * **Company Information:** Contains all related company info.
        * **Historical Charts:** Last 10 years of historical prices with interactive candlestick and line charts.
        * **Download Data:** Easily download historical prices for further analysis.
        * **Financials:** Access all quarterly and annual Financial Results, Balance Sheets, Cash Flows, and Splits & Dividends.
        ''')

    with tab2:
        st.markdown('''
        * **Comprehensive Indicators:** Contains all the famous indicators that traders and investors use while investing.
        * **Variety:** More than 11 indicators are available to apply directly on stock charts.
        ''')

    with tab3:
        st.markdown('''
        * **Important Metrics:** Contains all the crucial parameters and metrics related to a company.
        * **Breakout Signals:** Automatically detects if a selected stock is breaking out. Traders usually do this manually by analyzing candles!
        * **Consolidation Signals:** Along with breakouts, it gives signals for whether a stock is consolidating.
        ''')

    with tab4:
        st.markdown('''
        * **Automated Scanning:** Automate the manual process of looking at individual candles.
        * **Candlestick Patterns:** Scans all candlestick patterns for the selected stock.
        * **Trading Signals:** Generates clear signals whether the current pattern is bullish or bearish.
        ''')

    with tab5:
        st.markdown('''
        * **Advanced Modeling:** An efficient Machine Learning model built to predict the next day's price.
        * **Trained Data:** Our model is trained on the past 5 years of historical data.
        * **Short-term Memory:** While predicting, it looks closely at the past 2 months to effectively predict the next-day price.
        ''')

    st.divider()

    # Additional Information in Expander
    with st.expander("ℹ️ Data Information & Methodology"):
        st.markdown('''
        * **Data Source:** All the historical prices for the last 10 years are taken from Yahoo Finance.
        * **Storage:** Data is securely stored and pushed to a local SQLite database for fast retrieval.
        * **Timeframe:** The default time period for stock prices and model calculations is **one day** (Daily timeframe).
        ''')
...
#company info
 ...
 # closing price chart
    ...
    hist_price = pd.DataFrame(cursor.fetchall(),
                          columns=['Date', 'symbols', 'Open', 'High', 'Low', 'Close', 'Close', 'Volume'])
...
if (feature == 'Pattern Recognition'):
    ...
    tmp_df = df.drop(['Date', 'symbols', 'Open','High', 'Low', 'Close', 'Close', 'Volume'], axis=1,)
    tmp_df_1 = tmp_df.T
